...
const createHeaderStyles = createUseStyles(() => ({
  header: {
    ...
    right: (menuRightIndent) => menuRightIndent,
...
export const Header = () => {
  ...
  React.useEffect(() => {
    ...
    if (menuHtmlEls.current[0]) {
      ...
      setMenuRightIndent(winWidth / 2 - menuHtmlEls.current[menuHtmlEls.current.length - 1].offsetWidth / 2);
    }
  });
  
  const classes = createHeaderStyles(setMenuLeftIndent, setMenuRightIndent);
