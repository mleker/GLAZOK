...
const scrollHorizontally = (e, itemsWrapper, items, setCurrentItem, winCenter) => {
  e.preventDefault();
  const delta = Math.max(-1, Math.min(1, e.wheelDeltaY));
  itemsWrapper.scrollLeft -= delta * 5;
  for (const i = 0; i < items.length; i++) {
     const itemX = items[i].getBoundingClientRect().x;
     const itemWidth
     if (x <= winCenter && x > winCenter - )
  }
}
...
export const Header = ({ items }) => {
  ...
  React.useEffect(() => {
    window.addEventListener('mousewheel', (e) => scrollHorizontally(e, menuWrapperHtmlEl.current, menuHtmlEls.current), false);
