...
const getCurrentItem = (items, winCenter) => {
  for (let i = 0; i < Object.keys(items).length; i++) {
    ...
    if (rect.x + rect.width / 2 === winCenter) {
      return i;
    }
  }
}
...
export const Header = ({categories}) => {
  ...
  const onMouseWheel = (e) => {
    ...
    // }
    for (let i = 0; i < Object.keys(menuHtmlEls.current).length; i++) {
      const rect = menuHtmlEls.current[i].getBoundingClientRect();
      // if (rect.x <= winCenter + 15 && rect.x >= winCenter - rect.width - 15) {
      console.log('i:', i);  
      console.log('rect.x + rect.width / 2', rect.x + rect.width / 2);
      console.log('winWidth / 2', rect.x + rect.width / 2);
      if (rect.x + rect.width / 2 === winWidth / 2) {
        navigate(categories[i].custom_url, { replace: true })
      }
    }
  }
  ...
  React.useEffect(() => {
    ...
    categories && categories.map((category, i) => {
      if (category.custom_url === pathname) {
        setCurrentItem(i)
      }
    });
  }, []);
