...
export const Header = () => {
  ...
  const [menuIndents, setMenuIndents] = React.useState(null);
  const menuHtmlEls = React.useRef({});
  const menuLeftIndent = menuHtmlEls.current[0] && winWidth / 2 - menuHtmlEls.current[0].offsetWidth / 2;
  const menuRightIndent = menuHtmlEls.current[0] && winWidth / 2 - menuHtmlEls.current[menuHtmlEls.current.length - 1].offsetWidth / 2;
  ...
  React.useEffect(() => {
    ...
    if (menuHtmlEls.current[0]) {
      setMenuLeftIndent(menuLeftIndent1);
    } 
  });
