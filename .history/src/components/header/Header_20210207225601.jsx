...
import { Link as Link1 } from '../link/Link';
...
export const Header = ({ categories, initialCurrentItem, onMenuClick }) => {
  ...
  const onMouseWheel = (e) => {
    ...
    if (currentItem !== newCurrentItem) {
      ...
      navigate(categories[newCurrentItem].custom_url)
    }
  }
  ...
  const classes = createHeaderStyles({ menuLeftIndent, menuRightIndent, background: theme.background, color: theme.color });
