...
const createHeaderStyles = createUseStyles(() => ({
  header: {
    fontSize: 57,
    position: 'relative',
    left: (menuHtmlEls[0]) => menuLeftIndent,
...
export const Header = () => {
  ...
  const [menuIndents, setMenuIndents] = React.useState([]);
  ...
  React.useEffect(() => {
    ...
    if (menuHtmlEls.current[0]) {
      console.log('4');
      setMenuIndents([
        winWidth / 2 - menuHtmlEls.current[0].offsetWidth / 2,
        winWidth / 2 - menuHtmlEls.current[menuHtmlEls.current.length - 1].offsetWidth / 2,
      ]);
    }
  });
  
  const classes = createHeaderStyles(menuHtmlEls[0]);
