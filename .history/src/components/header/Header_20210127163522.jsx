...
export const Header = () => {
  ...
  React.useEffect(() => {
    getCategories(devApiUrl)
      .then((data) => {
        setCategories(data);
        setCurrentItem(data.find(item => item.custom_url === pathname).id - 1)
      })
      .catch(console.log);
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (menuHtmlEls.current[0]) {
      console.log('currentItem', menuHtmlEls.current[currentItem]);
      // console.log('currentItem', currentItem);
      setMenuLeftIndent( menuHtmlEls.current[currentItem].offsetLeft - winWidth / 2 - menuHtmlEls.current[currentItem].offsetWidth / 2);
