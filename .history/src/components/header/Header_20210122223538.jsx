...
const createHeaderStyles = createUseStyles(() => ({
  ...
  div1: {
    width: (menuLeftIndent) => menuLeftIndent,
    display: 'inline-block',
  },

  div2: {
    width: (menuRightIndent) => menuRightIndent,
    display: 'inline-block',
  },
...
export const Header = () => {
  ...
  React.useEffect(() => {
    ...
    if (menuHtmlEls.current[0]) {
      console.log(menuHtmlEls.current[Object.keys(menuHtmlEls.current).length - 1].offsetWidth / 2);
      setMenuLeftIndent(winWidth / 2 - menuHtmlEls.current[0].offsetWidth / 2);
      setMenuRightIndent(winWidth / 2 - menuHtmlEls.current[Object.keys(menuHtmlEls.current).length - 1].offsetWidth / 2);
    }
  });
