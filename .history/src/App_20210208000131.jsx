    ...
    <>
      {categories && (
        <ThemeContext.Provider value={{ theme, setTheme }}>
          <Location>
            {({ location }) => (
              <CSSTransition key={location.key} classNames="fade" timeout={500}></CSSTransition>
              <Router location={location} className={classes.router}>
                <RootPage
                  path={createHomeUrl()}
                  categories={categories}
                >
                  
                    {categories.map((category, i) => <Category path={category.custom_url} key={i} category={category} />)}
